function plot_diag(fname,outdir,vars,tits)
...
cmd = ['rm plots/',outdir,'/diag*png']; system(cmd);
...
for i = 1:length(vars)
    ...
    l.AutoUpdate = 'off';
    l.Box = 'off';
	this_var = squeeze(diag.(vars{i}));
	hold on
	tcnt = 1;
	for t = 1:dt:nrec
		if t == 1
			plot(diag.(vars{i})(t,:),-grid.z_r,'--','linewidth',0.5,'color',rgb('Black'));
		else
			plot(diag.(vars{i})(t,:),-grid.z_r,'-','linewidth',0.5,'color',old_cmap(tcnt,:));
			tcnt = tcnt + 1;
		end
	end
	plot(diag.(vars{i})(end,:),-grid.z_r,'-','linewidth',0.5,'color',rgb('Black'));
	xlim([lims{i}]);
...
export_fig('-png',['plots/',outdir,'/diagnostic'],'-m5','-p0.05');
close all
